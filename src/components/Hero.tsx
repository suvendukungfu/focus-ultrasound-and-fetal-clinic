  ...
  return (
    <section ref={containerRef} className="relative min-h-[100dvh] flex flex-col justify-center overflow-hidden pt-20 bg-background/30">
      ...
      <motion.div style={{ y: y1, opacity }} className="absolute inset-0 z-0">
        ...
        />
        {/* Mobile: heavier left gradient so text is legible; desktop: same as before */}
        <div className="absolute inset-0 bg-gradient-to-r from-background/80 via-background/50 to-background/20 md:from-background/70 md:via-background/10 md:to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent opacity-60 md:opacity-30" />
      </motion.div>

      {/* Background glow blobs — hidden on small screens to reduce paint cost */}
      <div className="hidden md:block absolute top-40 right-20 w-96 h-96 bg-primary/10 rounded-full blur-[100px] pointer-events-none" />
      <div className="hidden md:block absolute bottom-40 left-20 w-80 h-80 bg-secondary/10 rounded-full blur-[100px] pointer-events-none" />

      <div className="relative z-10 w-full max-w-screen-xl mx-auto px-5 sm:px-8 py-10 md:py-20 flex-1 flex flex-col justify-center">
        <div className="grid lg:grid-cols-12 gap-6 md:gap-12 items-center">
          ...
          <div className="lg:col-span-7 xl:col-span-6">
            ...
            <MotionReveal delay={0.2} duration={0.8}>
              <h1 className="font-display text-[2rem] sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-slate-950 dark:text-white leading-[1.1] mb-4 md:mb-8 max-w-[18ch] sm:max-w-[15ch] md:max-w-2xl drop-shadow-sm">
                {language === 'en' ? (
                  <>
                    Ultrasound & <span className="text-medical-teal">Fetal Medicine</span> Clinic <br />
                    <span className="text-lg sm:text-xl md:text-3xl lg:text-4xl block mt-2 md:mt-4 font-medium text-slate-700 dark:text-slate-300">in Greater Noida West</span>
                  </>
                ) : (
                  <>
            ...
            <MotionReveal delay={0.4} duration={0.8}>
              <p className="text-slate-800 dark:text-slate-200 text-sm sm:text-base md:text-xl font-body leading-relaxed mb-6 md:mb-10 max-w-2xl drop-shadow-sm">
                {language === 'en' 
            ...
            <MotionReveal delay={0.6} duration={0.8}>
              <div className="flex flex-col xs:flex-row gap-3 md:gap-4 mb-8 md:mb-12">
                <Link
                  to="/contact"
                  className="bg-medical-teal text-white hover:bg-medical-teal/90 px-5 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl font-semibold shadow-glow flex items-center justify-center gap-2 transform transition-transform hover:-translate-y-1 text-sm md:text-base"
                >
                ...
                <a
                  ...
                  className="bg-green-500 text-white hover:bg-green-600 px-5 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl font-semibold shadow-glow flex items-center justify-center gap-2 transform transition-transform hover:-translate-y-1 text-sm md:text-base"
                >
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-4 border-t border-slate-100 dark:border-white/10 pt-6 md:pt-10">
                {[
                  ...
                  <div key={m.label} className="flex items-center gap-2.5">
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-medical-soft dark:bg-slate-900/40 flex items-center justify-center flex-shrink-0">
                      <m.icon className="w-4 md:w-5 h-4 md:h-5 text-medical-teal" />
                    </div>
                    <div className="flex flex-col">
                      <span className="font-display text-base md:text-xl font-bold text-slate-950 dark:text-white leading-tight">{m.value}</span>
                      <span className="text-slate-600 dark:text-slate-400 text-[10px] md:text-sm font-medium">{m.label}</span>
                    </div>
                  </div>
                ))}
              </div>
