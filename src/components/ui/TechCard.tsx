...
const TechCard: React.FC<TechCardProps> = ({ 
  ...
  return (
    <motion.div
      ...
      className={`
        flex flex-col lg:flex-row gap-12 lg:gap-20 items-center overflow-hidden rounded-[3rem] p-6 transition-all duration-700
        ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}
        ${theme === 'dark' ? 'bg-[#0f172a]/40 border-white/5 shadow-elevated' : 'bg-white shadow-medium'}
        border group
      `}
    >
      {/* Image Side with Scale-on-Hover */}
      <div className="w-full lg:w-1/2 relative">
        <div className="absolute -inset-4 bg-primary/5 rounded-[3rem] blur-2xl transition-all duration-700 group-hover:bg-primary/10" />
        <div className="relative aspect-[16/10] rounded-[2.5rem] overflow-hidden border border-border/50 shadow-medium">
          <img 
            src={image} 
            alt={name}
      ...
      <div className="w-full lg:w-1/2 lg:p-12">
        <motion.div 
          ...
          whileHover={{ x: 8 }}
        >
          <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20 shadow-soft group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-500">
        ...
        <p className={`
          font-body text-xl leading-relaxed mb-12 transition-colors duration-500
